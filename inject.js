...
function main() {
    ...
    ret = replaceNodeText(document.body);
    var opinions = ret.opinions;
    var total = ret.total;

    var title = document.getElementsByTagName('h1')[0]
    if (!title) { return; }
    if (opinions >= 2 && opinions < 6) {
      title.style.color = '#661414';
    } else if (opinions >= 6 && opinions < 11) {
      title.style.color = '#a62121';
    } else if (opinions >= 11) {
      title.style.color = '#fc3232';
    }
}
...
function replaceNodeText(node) {
    ...
    while(textNode = walker.nextNode()) {
        total++
        if (countOpinions(textNode.nodeValue)) {
          textNode.parentNode.style.color = 'red';
          opinions++;
        }
    }
    ret = {
...
function countOpinions(text) {
    const opinionWords = largeArr()
    var firstChar = text[0]

    // just because of the way the array is set up
    text = '        ' + text;

    var relevantArr = opinionWords[firstChar]
    if (relevantArr && relevantArr.includes(text)) {
      return true;
    }
    return false;
}
